...
All operations use the browser's built-in **Web Crypto API** (`SubtleCrypto`). There are no runtime crypto dependencies. ML-KEM-768 is bundled at build time from the [`mlkem`](https://www.npmjs.com/package/mlkem) package.
...
Hybrid V2 is **enabled by default**: the ML-KEM-768 implementation (`mlkem`, MIT) is
bundled directly into `background-bundle.js` at build time. New identities
automatically get an ML-KEM-768 key, and messages upgrade to V2 whenever both parties
advertise one. Existing V1 messages keep working. There is no separate runtime
dependency and no remote code.

To rebuild the bundle after changing the background sources or updating `mlkem`:

```bash
npm install
npm run build         # bundles mlkem into src/background-bundle.js, then packs both browsers
```
...
├── scripts/                     # bundle / sync / pack + LibreWolf helpers
...
**Why a generated bundle?** ES-module service workers work in Chrome but are unreliable in Firefox MV3. `background-bundle.js` is a single self-contained classic (non-module) IIFE built from the ES-module sources via esbuild — engine, keystore, handler, and ML-KEM. It is loaded directly (Chrome `service_worker`, Firefox `scripts[]`) rather than via `importScripts()`, which is unavailable in Firefox MV3 background event pages.
